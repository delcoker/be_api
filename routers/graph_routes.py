...
def get_issue_of_severity_chart(req: Request, start_date: datetime = Form(...), end_date: datetime = Form(...), granularity: str = Form(...), db: Session = Depends(get_db)):
    graph_result = graphs_controller.issue_of_severity(db, start_date, end_date, granularity, req.headers['token'])
    return graph_result
