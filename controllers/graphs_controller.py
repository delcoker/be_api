...
def issue_of_importance(start_date: str, end_date: str, granularity: str):
        date_format, group_by_clause = get_date_granularity(granularity)

        issue_of_importance_data = issue_of_importance_chart(date_format, start_date, end_date, group_by_clause)

        charts = {"charts": [issue_of_importance_data]}

        return charts

def issue_of_importance_chart(date_format, start_date, end_date, group_by):
        category_names = []
        importance = []
        sql = "SELECT categories.category_name, COUNT(post_id) as 'importance' " \
              ...
              "WHERE created_at between '{}' and '{}' GROUP BY categories.category_name " \
              "ORDER BY importance DESC".format(start_date, end_date)
...
def issue_of_severity(start_date: str, end_date: str, granularity: str):
        date_format, group_by_clause = get_date_granularity(granularity)

        issue_of_importance_data = issue_of_severity_chart(start_date, end_date, group_by_clause)

        charts = {"charts": [issue_of_importance_data]}

        return charts

def issue_of_severity_chart(start_date, end_date, group_by):
        categories_name = []
        positive_data = {}
        negative_data = {}
        neutral_dict = {}
        ...
        sql = "SELECT categories.category_name, COUNT(post_id) as 'importance', sentiment_score " \
              "FROM post_data_categorised_view " \
              "JOIN categories ON post_data_categorised_view.category_id = categories.id " \
              "WHERE created_at between '{}' and '{}' GROUP BY categories.category_name, sentiment_score, {} " \
              "ORDER BY importance DESC".format(start_date, end_date, group_by)
